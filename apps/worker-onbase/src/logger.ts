...
/**
 * The OnBase worker's logger — the same stack and env contract as
...
export async function attachPersistentLogging(): Promise<void> {
  ...
  if (!dbResult.ok) {
    logger.error('Could not attach Postgres log adapter; logging to console only', {
      component: 'worker-onbase/logging',
  ...
  try {
    ...
    console.error(
      `FATAL [worker-onbase/logging]: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
...
function attachHttpShipping(endpoint: string): void {
  ...
  const adapter = new HttpAdapter({
    ...
    encryption: signingKeys
      ? { clientId: process.env.LOG_SHIP_CLIENT_ID ?? 'renkei-worker-onbase', signingKeys }
      : {},
    onError: (err) => {
      ...
      console.error('[worker-onbase/log-ship] delivery failed', err);
    },
  });
  adapter.start();
  logger.addAdapter(adapter);
