...
function promptOf(text: string, tools: ToolDescriptor[]): string {
  const toolLines = tools
    ...
    .join('\n');
  const varLines = BUILTIN_VARIABLES.map(
    (variable) => `- ${variable.name}: ${variable.description}`
  ).join('\n');

  return [
    'A user described an automation in plain words. Split it into ordered steps for a step-runner with these rules:',
    ...
    '{"name": "short agent name", "steps": [{"name": "short step name", "instruction": "plain words with {{tool:...}} and {{var:...}} tokens", "tool": "tool_name or null", "saveAs": "name or null"}]}',
  ].join('\n');
}
...
export async function draftAgentFromProse(
  ...
  const completion = await Promise.race([
    llm.provider.complete({
      ...
      messages: [
        {
          role: 'user',
          content: [{ type: 'text', text: promptOf(text.slice(0, MAX_PROSE_CHARS), tools) }],
        },
      ],
      tools: [],
  ...
  const knownVars = new Set(BUILTIN_VARIABLES.map((variable) => variable.name));

  const steps: AgentStep[] = [];
  for (const entry of parsed.steps.slice(0, 20)) {
    if (typeof entry !== 'object' || entry === null) continue;
    const step: { name?: unknown; instruction?: unknown; saveAs?: unknown } = entry;
    ...
    const saveAs =
      typeof step.saveAs === 'string' && step.saveAs.trim() ? step.saveAs.trim() : undefined;
