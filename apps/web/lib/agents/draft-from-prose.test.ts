...
describe('drafting a needsApproval gate', () => {
  it('offers the gate vocabulary on an action step', async () => {
    ...
    expect(prompt).toContain('approvalTimeoutHours');
  });

  it('sets needsApproval and approvalTimeoutHours on the tool step', async () => {
    replies = [
      JSON.stringify({
        name: 'Refund triage',
        steps: [
          {
            name: 'Refund',
            instruction: 'Refund the customer with {{tool:jira_search_issues}}',
            tool: 'jira_search_issues',
            needsApproval: true,
    ...
    expect(step.approvalTimeoutHours).toBe(48);
  });

  it('drops the gate with a soft problem when the step has no tool', async () => {
    replies = [
      JSON.stringify({
        name: 'Reasoning',
        steps: [
          {
            name: 'Think',
            instruction: 'Decide what to do next',
            needsApproval: true,
          },
        ],
      }),
    ...
    const step = actionOf(result.steps[0]);
    expect(step.needsApproval).toBeUndefined();
    expect(JSON.stringify(requests[1]?.messages ?? '')).toContain('needsApproval');
  });
});
