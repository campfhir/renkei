...
const zero: UsageBuckets = { today: 0, week: 0, month: 0, quarter: 0, year: 0, allTime: 0 };
...
describe('labelStepUsage', () => {
  it('names top-level and nested steps from the current definition', () => {
    const labeled = labelStepUsage(doc, [
      row('11111111-1111-4111-8111-111111111111'),
      row('33333333-3333-4333-8333-333333333333'),
    ]);
    expect(labeled.map((entry) => entry.stepName)).toEqual(['Read the inbox', 'Triage it']);
  });
  ...
  it('leaves a removed step and spend outside any step unnamed', () => {
    ...
    expect(labeled.map((entry) => entry.stepName)).toEqual([null, null]);
