...
export function registerAgentTools(server: McpServer, context: MCPToolContext): void {
  ...
  const STEP_NODE_GRAMMAR = [
    ...
    'LIST a foreach loop can iterate. FIELDS ARE FIXED AT AUTHORING TIME — the same labels',
    'every run, never computed live. A node inside a LOOP that needs to ask something',
    'DIFFERENT each round (about the CURRENT item) cannot bind that to a field — put it in',
    '`message` instead, as a variable a reasoning step wrote fresh that round (see the var-',
    'segment note at the end of this grammar); reserve `fields` for whatever about the',
    'ANSWER stays the same shape every round, like an issue key or a number),',
    ...
    '{t:"tool", name:"<a skill name>"}. A var segment renders that variable\'s value',
    'VERBATIM, with no formatting in between. In an approval or terminal `message` — the',
    'ONLY segment kinds a PERSON reads — that means naming a variable already written FOR a',
    'person, never a raw record another step saved for a LATER step to parse (a raw loop',
    'item, a pipe-delimited or JSON-shaped hand-off). Add a `tool: null` reasoning step',
    'right before the approval/terminal node that turns the raw value into a one- or two-',
    'sentence plain-prose summary under its own name, and put THAT name in the message —',
    'this is exactly what a node inside a loop needs for a per-round question (see FIELDS',
    'ARE FIXED AT AUTHORING TIME above).',
  ].join(' ');
