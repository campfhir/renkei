/**
 * Start an Entra Developer user-grant flow: "let Renkei provision Entra
 * applications as me." The microsoft authorize route for the SECOND Entra
 * app registration (lib/entra-developer-app.ts): signed-in caller,
 * single-use state carrying the subject, provider recorded so the shared
 * callback knows which app's token endpoint to visit. The authority is the
 * org's own directory, never `common`.
 */
...
export async function GET(
  ...
  if (!session) {
    return NextResponse.json(
      { error: 'Not signed in', error_description: 'Sign in before connecting Entra Developer' },
      { status: 401 }
    );
  }
  ...
  const app = await getEntraDeveloperApp(tenantId, originResult.val);
  if (!app) {
    return NextResponse.json(
  ...
  // here, not in the picker. Required scopes always ride along.
  ...
  if (requestedParam) {
    ...
    effectiveScopes = [
      ...requested,
      ...ENTRA_DEVELOPER_REQUIRED_SCOPES.filter((scope) => !requested.includes(scope)),
    ].join(' ');
  }
  ...
  await db
    ...
    .values({
      ...
      provider: ENTRA_DEVELOPER_CONNECTOR,
