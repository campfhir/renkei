/**
 * Org-admin configuration of the SECOND Atlassian OAuth app ("Renkei JSM":
 * JSM + Ops scopes on their own app registration, because all-of scope
 * enforcement times the consent-URL length cliff makes the combined union
 * unfittable on one app). Same shape as the atlassian connector route.
 */
...
import { ATLASSIAN_JSM_CONNECTOR, DEFAULT_ATLASSIAN_JSM_SCOPES } from '@/lib/atlassian-app';
...
export async function GET(
  ...
  return NextResponse.json({
    connector: ATLASSIAN_JSM_CONNECTOR,
...
export async function PUT(
  ...
  const scopes =
    typeof body.scopes === 'string' && body.scopes ? body.scopes : DEFAULT_ATLASSIAN_JSM_SCOPES;
  const redirectUri =
