/**
 * Org-admin configuration of the FIFTH Atlassian OAuth app ("Renkei Jira
 * Admin": Jira administration on its own app registration, with classic
 * scopes — see lib/atlassian-scopes.ts for why it cannot share the Jira
 * app). Same shape as the atlassian-confluence connector route.
 */
...
import { ATLASSIAN_ADMIN_CONNECTOR, DEFAULT_ATLASSIAN_ADMIN_SCOPES } from '@/lib/atlassian-app';
...
export async function GET(
  ...
  const configResult = await getConnectorConfig(tenantId, ATLASSIAN_ADMIN_CONNECTOR, keyResult.val);
  if (!configResult.ok) {
  ...
  return NextResponse.json({
    connector: ATLASSIAN_ADMIN_CONNECTOR,
...
export async function PUT(
  ...
  const scopes =
    typeof body.scopes === 'string' && body.scopes ? body.scopes : DEFAULT_ATLASSIAN_ADMIN_SCOPES;
  const redirectUri =
  ...
  const existing = await getConnectorConfig(tenantId, ATLASSIAN_ADMIN_CONNECTOR, keyResult.val);
  ...
  invalidateConnectorConfigCache(tenantId, ATLASSIAN_ADMIN_CONNECTOR);
  return NextResponse.json({
    connector: ATLASSIAN_ADMIN_CONNECTOR,
    configured: true,
    enabled,
