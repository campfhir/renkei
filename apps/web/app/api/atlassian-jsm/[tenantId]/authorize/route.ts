...
/**
 * Authorize against the second Atlassian app ("Renkei JSM": JSM + Ops
 * scopes on their own grant — the split exists because all-of scope
 * enforcement × the consent-URL length cliff makes the combined union
 * unfittable on one app). Same shape as the Jira authorize route: the user
 * may narrow the org ceiling, never widen it, and the pending row records
...
export async function GET(
  ...
  try {
    ...
    const app = await getAtlassianJsmApp(tenantId, originResult.val);
    if (!app) {
      return NextResponse.json(
        { error: 'Atlassian JSM connector not configured for this organization' },
        { status: 503 }
      );
    }
    ...
    logger.error('Atlassian JSM authorize error: {error}', {
      component: 'auth/oauth',
      tenantId,
