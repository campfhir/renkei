/**
 * Disconnect the caller's grant on Renkei's GitHub App. Subject-scoped:
 * the session decides whose grant dies, never a parameter.
 */
...
import { deleteGrant, GITHUB } from '@renkei/provider-grants';
...
export async function DELETE(
  ...
  const deleted = await deleteGrant(GITHUB, tenantId, grant.provider_account_id);
  if (!deleted.ok) {
