...
/**
 * The live body of the browser-secrets card — where a person hands the
 * sandbox browser a login it may TYPE on their behalf without the model
 * ever seeing it. Adding one asks for a name, the hosts it may be typed
 * on, and its fields (username/password by default); Renkei seals it
 * under a passphrase it generates and shows ONCE here — copy it into a
 ...
 *
 * Split out of sandbox-secrets.tsx so that file's outer card chrome can be
 * a server component; the "Add secret" button lives in the same header row
 * as the title here rather than there, since it depends on the `adding`
...
export default function SandboxSecretsBody({
  ...
  const addAnchor = useCoachAnchor('secrets-add');
