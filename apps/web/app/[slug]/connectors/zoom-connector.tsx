...
export default function ZoomConnector({
  ...
  // Only catalog options count as choices; the required scope
  // (user:read:user) rides along server-side and is not offered here.
  const ceilingSet = new Set(ceiling);
  const pickable = ZOOM_SCOPE_OPTIONS.filter((option) => optionWithin(option, ceilingSet));
  ...
  const authorizeUrl = `/api/zoom/${tenantId}/authorize?scopes=${encodeURIComponent(
    scopesOfOptions(ZOOM_SCOPE_OPTIONS, selectedIds).join(' ')
  )}`;
  ...
  async function disconnect() {
    ...
    setNotice(null);
    try {
      const response = await fetch(`/api/zoom/${tenantId}/grant`, { method: 'DELETE' });
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
      <div className="flex items-center justify-between gap-4">
        <h2 className="font-semibold">Zoom (your account)</h2>
        {connected ? (
          ...
          <details className="mb-3 rounded-lg border border-gray-200 p-3 dark:border-gray-800">
            ...
            <div className="mt-3">
              ...
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                to have — you can reconnect later to change it. Zoom&apos;s consent screen shows the
                permissions this selection requests; unchecked capabilities are also enforced by
                Renkei — their tools never register.
              </p>
            </div>
          </details>
          <a
        ...
        (confirming ? (
          <div className="mt-3 rounded-lg border border-red-300 p-3 dark:border-red-800">
            <p className="mb-3 text-sm">
              Disconnect your Zoom account? The Zoom MCP tools stop working until you reconnect.
            </p>
