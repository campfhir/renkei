...
export default function BitbucketConnector({
  ...
  const pickable = ATLASSIAN_BITBUCKET_SCOPE_OPTIONS.filter((option) =>
  ...
  const authorizeUrl = `/api/atlassian-bitbucket/${tenantId}/authorize?scopes=${encodeURIComponent(
    scopesOfOptions(ATLASSIAN_BITBUCKET_SCOPE_OPTIONS, selectedIds).join(' ')
  )}`;
  ...
  async function disconnect() {
    ...
    setNotice(null);
    try {
      const response = await fetch(`/api/atlassian-bitbucket/${tenantId}/grant`, {
        method: 'DELETE',
      });
        ...
        (confirming ? (
          <div className="mt-3 rounded-lg border border-red-300 p-3 dark:border-red-800">
            <p className="mb-3 text-sm">
              Disconnect Bitbucket? The Bitbucket tools stop working until you reconnect.
            </p>
