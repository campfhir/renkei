...
/**
 * The live, stateful part of the Mirth Connect card — where a person
 * connects each of the org's Mirth servers (dev, test, prod…) with their
 * OWN Mirth account, the same gesture as the file-shares card. The admin
 * registers where a server lives; this list is who you are on it.
 * Connecting validates the credential against the live server before
 * anything is stored (a wrong password fails here, not later).
 *
 ...
 *
 * Split out of mirth-connector.tsx so that file's header/description can be
 * a server component; every row here manages its own live state, so
 * there's no static content left to hoist once you're past the heading.
 */
