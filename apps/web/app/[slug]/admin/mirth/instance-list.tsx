...
/**
 * Mirth instance registry list plus one draft form for creation, with a
 * reachability test that runs BEFORE anything is saved. Editing an
 * existing instance happens on its own page; the draft here only ever
 * creates.
 */
...
export function probeText(probe: ProbeResponse): string {
  ...
  if (probe.version) return `Reachable — Mirth Connect ${probe.version}.`;
  return 'Reachable — a Mirth REST API is listening (it asked for a login, as expected).';
}
