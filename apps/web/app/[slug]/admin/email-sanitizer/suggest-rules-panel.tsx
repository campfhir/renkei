...
export default function SuggestRulesPanel({ slug }: { slug: string }) {
  ...
  return (
    <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950">
      ...
      <SampleAnalyzer slug={slug} />
    </div>
  );
}

/**
 * Paste one real email body (your own — a sent message works well) and the
 * model picks out the org-wide boilerplate: signature-block constants,
 * social-link rows, injected banners. Legal/confidentiality notices are
 * reported as already covered rather than proposed — the built-in footer
 * detector removes those wholesale.
 */
function SampleAnalyzer({ slug }: { slug: string }) {
  ...
  async function analyze() {
    ...
    setAnalysis(null);
    setAdded(new Set());
    const result = await sendJsonFull<SampleAnalysis>(
      `/api/admin/${slug}/email-sanitizer/suggest-from-sample`,
      'POST',
      { text: sample }
    );
    setBusy(false);
    if (result.error || !result.data) {
      setError(result.error ?? 'Could not analyze the sample');
      return;
    }
    setAnalysis(result.data);
  }

  async function add(index: number, phrase: string) {
    setAddBusy(index);
    setError(null);
    const result = await sendJsonFull(`/api/admin/${slug}/email-sanitizer/banners`, 'POST', {
      phrase,
    });
    setAddBusy(null);
    if (result.error) {
  ...
  return (
    <div className="mt-4 border-t border-gray-100 pt-4 dark:border-gray-900">
      <h3 className="text-sm font-semibold">Teach it from a sample</h3>
      <p className="text-xs text-gray-500 dark:text-gray-400">
        Paste one real email body — your own sent mail works well — and the model picks out the
        org-wide boilerplate (signature constants, social-link rows, injected banners) as phrases
        for the library. Confidentiality notices are already stripped and won&apos;t be proposed.
      </p>
      <textarea
        value={sample}
        ...
        className="mt-2 w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-mono text-xs dark:border-gray-700 dark:bg-gray-900"
      />
      <div className="mt-2">
        <button
          type="button"
          disabled={busy || sample.trim().length < 40}
      ...
      {analysis && (
        <div className="mt-3 space-y-3">
          {analysis.alreadyCovered.length > 0 && (
            <ul className="space-y-1 text-xs text-gray-500 dark:text-gray-400">
              {analysis.alreadyCovered.map((note) => (
                <li key={note}>✓ {note}</li>
              ))}
            </ul>
          )}
          {analysis.phrases.length === 0 ? (
            <p className="text-sm text-gray-500">
              No new phrases to add — the sample&apos;s boilerplate is already handled.
            </p>
          ) : (
            <ul className="divide-y divide-gray-100 dark:divide-gray-900">
              {analysis.phrases.map((suggestion, index) => (
                <li
                  key={suggestion.phrase}
                  className="flex flex-wrap items-center justify-between gap-2 py-2"
                >
                  <div className="min-w-0">
                    <p className="break-all font-mono text-xs">“{suggestion.phrase}”</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {added.has(index) ? (
                    ...
                    <button
                      type="button"
                      disabled={addBusy !== null}
                      onClick={() => void add(index, suggestion.phrase)}
                      className="rounded-md border border-blue-600 px-2.5 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50 disabled:opacity-50 dark:hover:bg-blue-900/20"
                    >
                      {addBusy === index ? 'Adding…' : 'Add phrase'}
                    </button>
                  )}
                </li>
              ))}
