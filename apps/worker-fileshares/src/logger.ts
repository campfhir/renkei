...
/**
 * The fileshare worker's logger — the same stack and env contract as
 * apps/worker's and apps/worker-agents' (console + Postgres/HTTP
 ...
 * mirrors apps/worker-agents/src/logger.ts; the three evolve together.
 */
...
export async function attachPersistentLogging(): Promise<void> {
  ...
  if (!dbResult.ok) {
    logger.error('Could not attach Postgres log adapter; logging to console only', {
      component: 'worker-fileshares/logging',
  ...
  try {
    ...
    console.error(
      `FATAL [worker-fileshares/logging]: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
...
function attachHttpShipping(endpoint: string): void {
  ...
  const adapter = new HttpAdapter({
    ...
    encryption: signingKeys
      ? { clientId: process.env.LOG_SHIP_CLIENT_ID ?? 'renkei-worker-fileshares', signingKeys }
      : {},
    onError: (err) => {
      ...
      console.error('[worker-fileshares/log-ship] delivery failed', err);
    },
  });
  adapter.start();
  logger.addAdapter(adapter);
