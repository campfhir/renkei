...
/**
 * The ADManager Plus worker's logger — the same stack and env contract as
 * apps/worker-mirth's and apps/worker's (console + Postgres/HTTP
 ...
 * mirrors apps/worker-mirth/src/logger.ts; the family evolves together.
 */
...
export async function attachPersistentLogging(): Promise<void> {
  ...
  if (!dbResult.ok) {
    logger.error('Could not attach Postgres log adapter; logging to console only', {
      component: 'worker-admanager/logging',
  ...
  try {
    ...
    console.error(
      `FATAL [worker-admanager/logging]: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
...
function attachHttpShipping(endpoint: string): void {
  ...
  const adapter = new HttpAdapter({
    ...
    encryption: signingKeys
      ? { clientId: process.env.LOG_SHIP_CLIENT_ID ?? 'renkei-worker-admanager', signingKeys }
      : {},
    onError: (err) => {
      ...
      console.error('[worker-admanager/log-ship] delivery failed', err);
    },
  });
  adapter.start();
  logger.addAdapter(adapter);
