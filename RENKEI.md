...
20. **Two worker processes over one events table, partitioned by lane.** _(Added 2026-08-13; amends the single-consumer reading of #17.)_ The `events` queue carries a `lane` column: `apps/worker`'s interactive process (replies, webhook orchestration, Graph/Zoom I/O — everything bounded by the connector clients' timeouts) claims only `lane = 'interactive'`; a second process from the same image (`embeddings-worker.ts`) claims only `lane = 'embedding'` and owns every ingest-time call to the org-configured embeddings endpoint — chunk-and-embed ingestion, index deletes/purges, and the asynchronous related-items back-fill on actionable items. Ingestion-heavy handlers enqueue `knowledge/*` events instead of embedding inline, so a slow or hung embeddings endpoint can never delay a reply, and a mailbox rebuild is per-item events rather than one monolith that outlives the queue's claim lease. Still `FOR UPDATE SKIP LOCKED`, still no new infrastructure. The embedding lane runs a **single consumer instance** on purpose: lane FIFO is what orders purge-before-re-ingest and delete-after-ingest; scaling that lane beyond one instance requires per-refId ordering first.
