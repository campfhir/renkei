...
21. **Agent-runner tokens are a distinct application class.** _(Added 2026-08-16.)_ The `oauth_access_tokens.application` column widens from `'jira'` to `'jira' | 'agent'`. An `'agent'` token is minted server-side by the agents worker for the lifetime of one agent run, bound to the run owner's subject, never issued through the OAuth authorization server, never refreshable, and revoked at run end (TTL as backstop). The MCP transport accepts both classes by looking each up explicitly — `resolveAccessToken` never matches across applications — so what a token class can reach is always a named decision at the verification site, not a property of the store. This is how user-drafted agents execute tools through the same gates (capability, scope, redaction, read-only, usage tracking) as any MCP client, acting as their owner.
22. **Agent run records carry content; the usage table stays content-free.** _(Added 2026-08-16.)_ `agent_run_steps.detail` stores resolved instructions, tool-call previews, and model summaries — a deliberate departure from `tool_calls`' content-free design (migration 032), because replay and debugging are the product feature of a run record and the owner drafts the agent knowing it records its work. The visibility split is an API-layer projection: content-free status/outcome/timing columns are org-admin-readable for every run; `detail` returns to the owner always, and to admins only for FAILED attempts (troubleshooting). Retention is an org policy (`agentRunRetentionDays`, default 30) enforced by a worker sweep.
