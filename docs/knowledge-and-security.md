...
- **Embeddings** (`embeddings.ts`): calls an org-configured, OpenAI-compatible `/embeddings` endpoint (connector key `embeddings`) — no embedding provider ships bundled. If an org hasn't configured one, the knowledge layer is simply off for that org.
- **Storage**: table `knowledge_chunks` (`tenant_id, provider, ref_id, content, metadata, embedding, source_at`). Chunk text is encrypted at rest via `@renkei/crypto`'s content envelope.
- **Retrieval** (`index.ts`, `searchKnowledge`/`listRecentKnowledge`): cosine-distance ANN over `knowledge_chunks`, overfetching `k*2` candidates, then piping every candidate through `@renkei/gates`'s `verifyCandidates` **before** decrypting content or returning anything. The index itself never authorizes — it only proposes candidates.
...
- Returns `{allowed, elided, unverified}` so a caller can report *why* something was withheld (denied outright vs. timed out) via `withheldNote()`, rather than silently returning fewer results.
...
3. **User expose/hide choices** (`hiddenCapabilities`): each employee can additionally hide a capability they *do* have provisioned.
