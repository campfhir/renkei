...
3. Apply org settings (`@renkei/settings`: read-only mode, disabled connectors, connector audiences) and the per-user capability projection (`@renkei/capability-registry`, see [`knowledge-and-security.md`](./knowledge-and-security.md)), built by `lib/mcp-tools/projection.ts` — the same constructor the tools page, the chat and the agent builder use through `lib/mcp-tools/tool-catalog.ts`. A connector an admin scoped to an audience registers nothing for a caller outside it; the caller's share is resolved from their recorded identity (`identities.idp_groups`, captured at sign-in from the tenant's `groups_claim`), never from the token, so an agent run is gated exactly as the person it runs for (`lib/connectors/audience.ts`, failing closed).
...
`lib/mcp-tools/` has one subdirectory per tool family — `agents`, `bitbucket`, `cards`, `confluence`, `fileshares`, `graph`, `jira`, `jira-service-management`, `knowledge`, `onbase`, `onedrive`, `outlook`, `sharepoint`, `summary`, `web-search`, `webex`, `zoom` — roughly 190 files total. Tool name prefixes: `jira_*`, `jsm_*`/`jsm_ops_*`, `webex_*`, `outlook_*`, `sharepoint_*`, `onedrive_*`, `confluence_*`, `zoom_*`, `fileshare_*`, `onbase_*`, plus the cross-cutting `search_knowledge`, `web_search` (the public web through the org's Azure OpenAI Responses API deployment and its built-in `web_search` tool — an org-wide connector an admin configures under Connector setup), `analyze_transcript`, and `whoami`. MCP Apps widget UI resources (interactive cards rendered inline in a client) are registered separately via `lib/mcp-tools/widgets.ts`, with widget source under `lib/mcp-widgets/`.
...
**Layer 1.5 — tenant sign-in.** Each tenant configures its own OIDC provider (`tenant_oidc`); users authenticate through it at `app/api/auth/oidc/{login,callback}` (`lib/oidc-discovery.ts`, `lib/oidc-id-token.ts`, `lib/oidc-roles.ts`). The OIDC `subject` is the identity key that binds sessions, MCP tokens, and provider grants together. The callback records the identity spine (`lib/identity.ts`: email, display name, and the raw values of the tenant's groups claim — `tenant_oidc.groups_claim`, default `groups` — replaced at every sign-in); the claim mappings are editable on Settings → Identity through `app/api/admin/[slug]/oidc-claims`.
