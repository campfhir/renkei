...
`onbase_admin_*` tools (`apps/web/lib/mcp-tools/onbase/admin-tools.ts`) wrap a SECOND, SEPARATE connector — `onbase-admin` — for OnBase's Administration API (`docs/onbase-administration-openapi-spec.json`), which is where document types and keyword types are actually created and configured; the Document API above only ever reads that vocabulary. Users and user groups are readable there too (lookup by name, memberships, what a group may see), and document types / document type groups can be granted to user groups — the grant is what makes a document type visible at all, OnBase Configuration included. Creating users or groups, memberships and privileges stay out. Same relationship as Jira/JSM/Confluence/Bitbucket under `connector-atlassian`: a different Hyland OAuth client, its own `connector_configs` row, its own `provider_grants` provider (`ONBASE_ADMIN`), its own capability gate — connecting `onbase` does not connect `onbase-admin`, or the reverse. `apps/worker-onbase` serves both from the same process; every op takes an optional `connector` field naming which row to resolve. See the design doc's "Admin tools" section.
...
The same worker also holds the checkouts behind **code projects** (opt-in with `SANDBOX_WORKSPACES_ENABLED`; see [`chat.md`](./chat.md#code-projects)): a Bitbucket repository cloned with a person's own grant, on a second volume, scoped to the project rather than to a person, where the project's chats list, search, read and edit files, run the project's own commands, commit, and push a branch back through the `code_*` local tools — never through the MCP surface. This is the one deliberate step past "curated verbs, not a shell": a command runs as the project's own unprivileged uid on the worker (the image's entrypoint keeps the supervisor root for exactly that), in a 0700 directory of its own, with an environment built from nothing plus the variables of the `.env` pasted on the project — values the worker seals under its own key, puts only in a command's environment, and masks out of every output, so the model sees names and never values. Git credentials ride one request as a header and land only in that git process's environment. See [`sandbox-workspaces-design.md`](./sandbox-workspaces-design.md), including the honest gap (a command has the container's network) and what that means for placement.

The package itself (`packages/connector-sandbox`) is dependency- and I/O-free, the `connector-onbase` shape: filename validation, quota/TTL constants, the egress guard, the browser snapshot vocabulary, and the workspace vocabulary (bounds, path/ref/command/variable validators, the per-caller uid derivation) — all the HTTP, disk, browser, git and process I/O lives in `apps/worker-sandbox`.
