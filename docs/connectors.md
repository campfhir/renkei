...
The same worker also serves **code workspaces** behind the `sandbox_workspace_*` tools (opt-in with `SANDBOX_WORKSPACES_ENABLED`): a Bitbucket repository a person cloned with their own grant, on a second volume, where a chat or an agent run can list, search, read and edit files, run the project's own commands, commit, and push a branch back. This is the one deliberate step past "curated verbs, not a shell": a command runs as the caller's own unprivileged uid on the worker (the image's entrypoint keeps the supervisor root for exactly that), in a 0700 directory of theirs, with an environment built from nothing plus the variables they supplied on the connectors page — values the worker seals under its own key, puts only in a command's environment, and masks out of every output, so the model sees names and never values. Git credentials ride one request as a header and land only in that git process's environment. See [`sandbox-workspaces-design.md`](./sandbox-workspaces-design.md), including the honest gap (a command has the container's network) and what that means for placement.
