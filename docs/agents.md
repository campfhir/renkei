...
- **Agent knowledge notes** — owner-curated. Every run's system prompt carries an INDEX of them (title and id per note, the whole body only for notes of 160 characters or fewer, up to `AGENT_NOTES_INJECT_MAX_CHARS` = 3000); the run fetches a full note with `agent_knowledge_list` when one looks relevant. Managed via the `agent_knowledge_*` MCP tools. Notes are rows in the knowledge index (provider `note`), so they carry the same lexical entry, context header and keywords as everything else — except that an agent writing a note supplies its own `keywords` in the tool call rather than having the org's default model asked again (it is a model already); omitted keywords are stored as none, which a reindex may fill later.
- **Agent memory** (`memory.ts`, table `agent_memories`) — what steps chose to remember. The only run-time writer is a step's `finish_step` call carrying `remember` (a fact future runs need and could not rediscover); the engine writes nothing on its own, so a run that remembers nothing leaves no trace here. An entry identical to one the agent already holds is skipped, and a note becomes visible to the later steps of the same run. Each entry is capped at `MEMORY_ENTRY_MAX_CHARS` (500); the system prompt carries up to `MEMORY_INJECT_MAX_CHARS` (4000), summary first, then newest-fit. Compaction (summarizing old memory to make room) happens in `apps/worker-agents` rather than in `packages/agents`, since it needs an LLM call. Selective deletion is available via the `agent_memory_forget` MCP tool.
...
Tool calls execute through `AgentMcpClient` (`mcp-client.ts`) against `apps/web`'s own MCP endpoint (`RENKEI_WEB_INTERNAL_URL`), authenticated with a per-run token (`token.ts`) — an agent run is, from the MCP gateway's point of view, just another authenticated caller subject to the same capability projection and gates as a human-driven MCP client. The token is minted with the names of the tools the run's steps and retry guidance reference (plus the notifier's own), and the gateway registers only those for it, so `tools/list` for a run is a handful of schemas and nothing a step did not name can be called.

What each model call carries is deliberate: the system prompt holds the run-constant context (the step frame, the owner's guardrails, the knowledge index, memory) and is byte-identical across the run, so it is a prompt-cache read from the second turn on; the per-step user message holds the instruction (long chip values render as `[name]` and are listed once under "Known information"), and lists only the variables the step references — builtins, the answers bound for it, the item vars of enclosing foreach loops, and what its chips name. A step's tool list is fixed for the whole attempt, with `resolve_time` offered only when the step is about time (`usesTime`). Because a step is given only what it chips, `packages/agents/src/lint.ts` (`lintAgentDraft`) flags prose that leans on a value without the chip — "comment on the ticket" with no `[the ticket]`, "reply to the message" with no trigger value — as amber hints in the builder (with a one-click "Add the chip" fix via `chipMention`) and as a "Worth a look" list in the MCP save tools' replies; hints never block a save. `finalize.ts` handles chaining a follow-up agent and reporting `agents/run.failed` back to `apps/worker`; `notifications.ts` and `memory-compaction.ts`/`maintenance.ts` cover run notifications and retention sweeps.
