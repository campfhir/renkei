...
## Failure capture, utilization, and the optimizer

Three pieces that turn run history into something an owner can act on:

- **Captured failures** (`agent_run_failures`, migration 079). `finalizeRun` in the engine writes one row per run that ends `failed`, via `captureAgentRunFailure` in `packages/agents/src/runs.ts`: the step it stopped at (resolved to its name at the time), the engine's `error_kind`, the failed attempt's `outcome_code`, the clipped run error, and what the run had cost (tokens, tool calls, attempts). The run id is a soft reference — runs are pruned by `agentRunRetentionDays`, failures by the org's `agentFailureRetentionDays` (default 90; `createFailureRetentionSweep` in `maintenance.ts`) — so "which step has this agent been failing on all quarter" stays answerable after the runs are gone. Never arguments, results, or transcripts; those stay on `agent_run_steps.detail` under its existing visibility rules.
- **My usage** (`apps/web/app/[slug]/utilization`). One person's overall utilization, pinned to the session's own subject: tokens and runs from `agent_run_counters` (their owned agents), tool calls from `tool_calls` under their subject — which includes their agents' calls, since a run token is bound to the owner — every series bucketed on the database session's calendar day (the one `agent_run_counters.day` is tallied on), and recurring failure signatures (same agent, same step, same kind) from the failures table, each linking to the agent's Improve panel. Queries live in `apps/web/lib/usage/user-utilization.ts`; the period/bucket arithmetic in `window.ts` is unit-tested.
- **The optimizer** (`agent_optimizations`, migration 080). Owner-only. `POST /api/tenant/{tenantId}/agents/{agentId}/optimize` writes a row (with the org's `agentOptimizerWindowDays`, default 30, frozen onto it) and enqueues an `optimize` job; the agents worker (`optimize.ts`, the draft handler's twin) calls back into `optimize/{optimizationId}/run` with an owner-bound token, and `apps/web/lib/agents/optimize.ts` gathers the evidence (captured failures, a per-step token/attempt profile from `agent_run_steps`, the failing steps of a few recent failed runs under the owner's visibility), renders the agent as the Markdown export does, and asks the org's model for a JSON report: summary, findings by area (accuracy, reliability, tokens) with a fix and the evidence each rests on, expected impact, and a **revision brief** in prose. The optimizer never edits the agent: `optimize/apply` turns the brief into an ordinary `agent_drafts` revision (the same pipeline as "describe it" in the builder), and the builder offers the result on open for the owner to review before saving. `optimization-sweep.ts` rescues stuck passes and prunes old reports, like the draft sweep.
