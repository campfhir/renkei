...
- **The token ledger** (`llm_calls`, migration 085): one row per finalized attempt's model spend (`purpose = 'run'`, with agent/run/step ids), per optimizer pass (`'optimize'`) and per chat turn (`'chat'`), attributed to the person whose spend it was. Since migration 096 each row also carries the **provider and model name** the tokens went to (and the `llm_model_configs` id as a soft reference) — the name, not just the id, because a config row can be re-pointed at another model and a million tokens on a small model and on a frontier one are not the same cost. Every writer has the resolved model in hand (`ResolvedLlm`) and passes it to `recordLlmCall`. Migration 097 adds the prompt-cache counts (`cache_read_input_tokens`, `cache_write_input_tokens`) beside `input_tokens`, which means the UNCACHED prompt — Anthropic's own accounting; the OpenAI adapter subtracts its `cached_tokens` so the three add up the same way from either provider. Pruned with the run log.
...
Every page that counts runs or tokens — the agent overview's Invocations and Usage panels, agent oversight, the person page, the Tools page's by-agent table — reads these ledgers. Agent oversight is a card per agent — runs, failures, tokens in with their cached part, tokens out — for one period at a time (today, yesterday, this week … all time), sortable by any of them, under an org card that splits the total by model; an agent's page (owner's and admin's) stacks cards for the same period: overall, by model, and by step behind a model filter, steps numbered as the outline numbers them (`getTokenUsageByModel`, `getAgentTokenUsageByStep` in `apps/web/lib/agents/agent-usage.ts`, step ids resolved to names and numbers against the current definition by `step-usage-labels.ts`). The per-day counters (`agent_run_counters`, 049/072) are no longer written or read: they were keyed on the database's `CURRENT_DATE` and could not be re-cut per viewer. Migration 087 backfills the ledgers from the runs still within retention, and leaves the counter table in place as pre-ledger history for an operator; a later migration may drop it.
