...
The per-day counters (`agent_run_counters`, 049/072) stay for the pages that already read them; they are keyed on the database's `CURRENT_DATE` and cannot be re-cut per viewer, which is why the new page reads the ledgers instead.
