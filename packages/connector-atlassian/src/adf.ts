 ...
 *
 * Lives in the connector package because BOTH sides need it: the MCP tools
 * render descriptions and comments for a model, and the knowledge sweep has
 * to flatten the same trees before embedding them. It used to live in
 * apps/web, which the worker cannot import — and the consequence was that the
 * sweep called `str()` on an ADF object, silently got an empty string, and
 * indexed every Jira issue without its description.
 *
 ...
 */
/** Local rather than imported: this module is shared with the worker, which
 * has no path into apps/web. */
