/**
 * The worker's one resolution step: from (tenant, instance, subject) to
 * the instance's connection details plus the CALLER'S OWN decrypted
 * credential. Only the ADManager Plus worker calls this — it is the only
 * process holding the encryption key for a purpose other than sealing on
 * the way in. Every uncertain outcome denies, and "no such instance" and
 * "not connected" are distinguished here but collapsed by the tools
 * before a model sees them (ids must not become an existence oracle).
 */
