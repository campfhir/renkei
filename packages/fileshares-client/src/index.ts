/**
 * The client for the fileshare worker (apps/worker-fileshares) — the ONLY
 * way any other process touches share bytes. SMB/SFTP sessions are heavy
 * I/O against servers with no self-defense, so they live in that dedicated
 * process; a caller of this client holds no protocol library and decrypts
 * no credential — the worker resolves the CALLER'S OWN stored credential
 * per call, which is why every function here carries the caller's
 * `subject`: the file server then authorizes each operation as that
 * account.
 *
 * Shared between apps/web (interactive fileshare_* tools, the files
 * browser REST routes) and apps/worker (the document-ocr-pipeline batch
 * handler reading source documents) — both need to reach the same
...
/**
 * One shared mapping from a client error to an HTTP answer, so a person in
 * the files browser and a model over MCP hear the same refusal. Resolution
 * tags get their own phrasings; backend tags fall back to the worker's
 * message. 'access_denied' is the file server's own verdict on the caller's
 * account — Renkei adds no verdicts of its own.
 */
