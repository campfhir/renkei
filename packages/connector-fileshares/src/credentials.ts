/**
 * A person's share credential, sealed at rest with the deployment key.
 *
 * One credential per (share, person) — the backend account IS the identity
 * the file server authorizes, so what a person can do on a share is exactly
 * what their own account can do. The document is a discriminated union
 * rather than a loose string map so a mis-filed field (an SFTP key on an
 * SMB share) is a parse error at read time instead of a confusing protocol
 * failure at connect time. Parsing fails closed: any malformed stored value
 * means the connection is unusable until the person reconnects, never a
 ...
 * for the fileshare worker's test-connection endpoint, which receives a
