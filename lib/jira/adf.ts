...
export function adfToMarkdown(document: unknown): string {
  ...
  return renderBlocks(childrenOf(node), 0).trim();
}
...
function renderMedia(node: AdfNode): string {
  ...
  // Deliberately not a link: attachment content endpoints are blocked by the
  // allowlist, so a URL here would be an invitation to a dead end.
...
function childrenOf(node: AdfNode): AdfNode[] {
  ...
  return content.filter((child): child is AdfNode => asNode(child) !== null);
}
